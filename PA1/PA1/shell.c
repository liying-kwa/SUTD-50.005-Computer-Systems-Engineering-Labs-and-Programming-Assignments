...
int shellFind(char **args)
{
  ...
  int returnVal;
  returnVal = execvp("./shellPrograms/find", args);
  // 2. Check if execvp is successful by checking its return value
  // 3. A successful execvp never returns, while a failed execvp returns -1
  // 4. Print some kind of error message if it returns -1
  if (returnVal == -1) {
    printf("execvp is unsuccessful. Returning to loop... \n");
  }
...
int shellDisplayFile(char **args)
{
  ...
  int returnVal;
  returnVal = execvp("./shellPrograms/display", args);
  // 2. Check if execvp is successful by checking its return value
  // 3. A successful execvp never returns, while a failed execvp returns -1
  // 4. Print some kind of error message if it returns -1
  if (returnVal == -1) {
...
int shellListDirAll(char **args)
{
  ...
  int returnVal;
  returnVal = execvp("./shellPrograms/listdirall", args);
  // 2. Check if execvp is successful by checking its return value
  // 3. A successful execvp never returns, while a failed execvp returns -1
  // 4. Print some kind of error message if it returns -1
  if (returnVal == -1) {
...
int shellListDir(char **args)
{
  ...
  int returnVal;
  returnVal = execvp("./shellPrograms/listdir", args);
  // 2. Check if execvp is successful by checking its return value
  // 3. A successful execvp never returns, while a failed execvp returns -1
  // 4. Print some kind of error message if it returns -1
  if (returnVal == -1) {
...
int shellCountLine(char **args)
{
  ...
  int returnVal;
  returnVal = execvp("./shellPrograms/countline", args);
  // 2. Check if execvp is successful by checking its return value
  // 3. A successful execvp never returns, while a failed execvp returns -1
  // 4. Print some kind of error message if it returns -1
  if (returnVal == -1) {
...
int shellSummond(char **args)
{
  ...
  int returnVal;
  returnVal = execvp("./shellPrograms/summond", args);
  // 2. Check if execvp is successful by checking its return value
  // 3. A successful execvp never returns, while a failed execvp returns -1
  // 4. Print some kind of error message if it returns -1
  if (returnVal == -1) {
...
int shellCheckDaemon(char **args)
{
  ...
  int returnVal;
  returnVal = execvp("./shellPrograms/checkdaemon", args);
  // 2. Check if execvp is successful by checking its return value
  // 3. A successful execvp never returns, while a failed execvp returns -1
  // 4. Print some kind of error message if it returns -1
  if (returnVal == -1) {
...
int shellExecuteInput(char **args)
{
  ...
  // 2. Otherwise, check if args[0] is in any of our builtin_commands, 
  // and that it is NOT cd, help, exit, or usage.
  int match = -1;
  for (int i = 0; i < numOfBuiltinFunctions(); i++) {
  ...
  if (match < 0) {
    // 7. If args[0] is not in builtin_command, print out an error message to tell 
    // the user that command doesn't exist and return 1
    ...
    if (pid < 0) {
      ...
      // 4. For the child process, execute the appropriate functions depending on the command 
      // in args[0]. Pass char ** args to the function.
      switch(match) {
        case 4:
      ...
      // 5. For the parent process, wait for the child process to complete and 
      // fetch the child's return value.
...
char **shellTokenizeInput(char *line)
{
  ...
  /** TASK 2 **/
  // 1. Allocate a memory space to contain pointers (addresses) to the 
  // first character of each word in *line. 
  // Malloc should return char** that persists after the function terminates.
  char **token_positions = malloc(sizeof(char*) * 8);

  // 2. Check that char ** that is returend by malloc is not NULL
  if (token_positions == NULL) {
    printf("token_positions is NULL");
  }
...
void shellLoop(void)
{
  ...
  int returnValue;
  returnValue = shellExecuteInput(args);
  ...
  free(args);
  // 9. check return value of shellExecuteInput. 
  // If 1, continue the loop (point 1) again and prompt for another input. Else, exit shell. 
  if (returnValue == 1) {
    shellLoop();
  } else {
    return;
...
int main(int argc, char **argv)
{
  ...
  // // Test Part 1: shellReadLine
  // char *line = shellReadLine();
  // printf("The fetched line is : %s \n", line);

  // // Test Part 2: shellTokenizeInput, type "listdir -a"
  // char **args = shellTokenizeInput(line);
  // printf("The first token is %s \n", args[0]);
  // printf("The second token is %s \n", args[1]);

  // // Test Part 3: shellExecuteInput, type "find file_1.txt"
