...
char *path = "/mnt/c/Code/PA1/ProgrammingAssignment1/logfile_test.txt";
...
static int create_daemon()
{
    ...
    // 3. On child process (this is intermediate process), call setsid() so that the child becomes 
    // session leader to lose the controlling TTY
    setsid();
    ...
    // 6. Child process (the daemon) set new file permissions using umask(0). 
    umask(0);

    // Daemon's PPID at this point is 1 (the init)
    // 7. Change working directory to root
    chdir("/");

    // 8. Close all open file descriptors using sysconf(_SC_OPEN_MAX) and 
